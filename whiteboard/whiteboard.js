...
function loadWorkflow() {
  ...
  const selectedWorkflow =
    workflowComboBox.options[workflowComboBox.selectedIndex].text;
...
function addNewColumn(name) {
  ...
  if (pos_options > 0) {
    ...
    const selectedPosition =
      pos_comboBox.options[pos_comboBox.selectedIndex].text;
...
function positionElement(stickyArea, posY) {
  const draggables = [
    ...stickyArea.querySelectorAll(".draggable:not(.dragging)"),
  ];
